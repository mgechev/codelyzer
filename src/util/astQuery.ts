...
export function callExpression(dec: ts.Decorator): Maybe<ts.CallExpression> {
  return Maybe.lift(dec.expression).fmap(expr => (isCallExpression(expr) ? (expr as ts.CallExpression) : undefined));
}
...
export function objectLiteralExpression(expr: ts.CallExpression): Maybe<ts.ObjectLiteralExpression> {
  return Maybe.lift(expr.arguments[0]).fmap(arg0 => (isObjectLiteralExpression(arg0) ? (arg0 as ts.ObjectLiteralExpression) : undefined));
}
...
export function withIdentifier(identifier: string): (expr: ts.CallExpression) => Maybe<ts.CallExpression> {
  return ifTrue((expr: ts.CallExpression) => isIdentifier(expr.expression) && expr.expression.text === identifier);
}
