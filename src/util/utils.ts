...
export const getDecorator = (node: ClassDeclaration, decoratorName: string): Decorator | undefined =>
  createNodeArray(node.decorators).find(
    (decorator) =>
...
export const getPipeDecorator = (node: ClassDeclaration): Decorator | undefined => getDecorator(node, 'Pipe');
...
export const getDeclaredInterfaces = (node: ClassDeclaration): NodeArray<ExpressionWithTypeArguments> => {
  ...
  return heritageClause ? heritageClause.types : createNodeArray([]);
};

export const getDeclaredInterfaceNames = (node: ClassDeclaration): string[] => getDeclaredInterfaces(node).map(getSymbolName);
