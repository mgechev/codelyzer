...
export const getDecoratorArgument = (decorator: Decorator): ObjectLiteralExpression | undefined => {
  ...
  if (!isCallExpression(expression) || !expression.arguments || expression.arguments.length === 0) return undefined;
...
export const getComponentDecorator = (declaration: ClassDeclaration): Decorator | undefined =>
  createNodeArray(declaration.decorators).find(
    d =>
      isCallExpression(d.expression) && d.expression.arguments && d.expression.arguments.length > 0 && getDecoratorName(d) === 'Component'
  );

export const getSymbolName = (expression: ExpressionWithTypeArguments): string => {
...
export const getDeclaredInterfaces = (node: ClassDeclaration): string[] => {
  const heritageClause = createNodeArray(node.heritageClauses).find(h => h.token === SyntaxKind.ImplementsKeyword);

  return heritageClause ? heritageClause.types.map(getSymbolName) : [];
};

export const getDeclaredLifecycleInterfaces = (node: ClassDeclaration): ReadonlyArray<LifecycleInterfaceKeys> =>
  getDeclaredInterfaces(node).filter(isLifecycleInterface) as ReadonlyArray<LifecycleInterfaceKeys>;
...
export const isSameLine = (sourceFile: SourceFile, pos1: number, pos2: number) =>
  getLineAndCharacterOfPosition(sourceFile, pos1).line === getLineAndCharacterOfPosition(sourceFile, pos2).line;

export const isStringLiteralLike = (node: Node) =>
...
export const toTitleCase = (value: string) => value.replace(unicodeWordMatch, val => val[0].toUpperCase() + val.slice(1).toLowerCase());
