...
export class MetadataReader {
  ...
  private contexts: ContextSymbols[];

  constructor(private program: ts.Program, fileResolver: FileResolver) {
    ...
    this.contexts = this.projectSymbols.getLazyLoadedContexts()
      .concat(this.projectSymbols.getRootContext());
  }

  read(d: ts.ClassDeclaration, fileName: string): DirectiveMetadata {
    let result: DirectiveSymbol = null;
    for (let i = 0; i < this.contexts.length; i += 1) {
      const currentContext = this.contexts[i];
      try {
        result = currentContext.getDirectiveFromNode(d, fileName);
        break;
      } catch (e) {}
    }
    if (!result) {
      ...
      const metadata = result.getNonResolvedMetadata();
    }
