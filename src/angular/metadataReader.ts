...
import { DirectiveMetadata, ComponentMetadata, CodeWithSourceMap, TemplateMetadata, StylesMetadata, StyleMetadata } from './metadata';
...
export class MetadataReader {
  ...
  protected readComponentMetadata(d: ts.ClassDeclaration, dec: ts.Decorator): ComponentMetadata {
    ...
    const template_M: Maybe<TemplateMetadata> = external_M.bind(external => this.readComponentTemplateMetadata(dec, external));
    const style_M: Maybe<StylesMetadata> = external_M.bind(external => this.readComponentStylesMetadata(dec, external));
